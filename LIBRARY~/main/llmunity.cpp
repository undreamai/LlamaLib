...
{
    std::cout<<params.logits_all<<" "<<params.embedding<<" "<<params.model<<" "<<params.n_ctx<<std::endl;
    if (params.logits_all) {
        throw std::runtime_error("please use the 'perplexity' tool for perplexity calculations");
    }

    if (params.embedding) {
        throw std::runtime_error("please use the 'embedding' tool for embedding calculations");
    }
...
void LLM::init(){
    ...
    if (!path_session.empty()) {
        ...
        if (!file_exists(path_session)) {
            ...
            if (!llama_load_session_file(ctx, path_session.c_str(), session_tokens.data(), session_tokens.capacity(), &n_token_count_out)) {
                throw std::runtime_error("error: failed to load session file " + path_session);
            }
    ...
    if ((int) embd_inp.size() > n_ctx - 4) {
        throw std::runtime_error("error: prompt is too long ("+std::to_string((int)embd_inp.size())+" tokens, max "+std::to_string(n_ctx - 4)+")");
    }
